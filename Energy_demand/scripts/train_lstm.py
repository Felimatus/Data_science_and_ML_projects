"""
Train an LSTM seq2seq model for electricity demand forecasting.

Architecture:
    Dilated causal Conv1D stack (multi-scale feature extraction)
    → LSTM (temporal modelling on compressed representations)
...
lstm_model = tf.keras.Sequential([
    ...
    # Receptive field: dilation 1+2+4+8 = 15 days → captures full bi-weekly
    # patterns before the LSTM sees anything.
    ...
    # --- Bidirectional LSTM on extracted features ---
...
# Train
# ---------------------------------------------------------------------------
results = fit_and_evaluate(lstm_model, train_ds, valid_ds,
                           learning_rate=0.5, model_name="lstm_seq2seq")
...
Y_pred = lstm_model.predict(last_X)
